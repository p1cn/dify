...
class CotAgentRunner(BaseAgentRunner, ABC):
    ...
    def run(
        ...
        iteration_step = 1
        max_iteration_steps = (
            max(app_config.agent.max_iteration if app_config.agent else 5, 5) + 1
        )
        ...
        def increase_usage(
            final_llm_usage_dict: dict[str, Optional[LLMUsage]], usage: LLMUsage
        ):
                    ...
                    try:
                        if isinstance(scratchpad.action.action_input, dict):
                            final_answer = json.dumps(
                                scratchpad.action.action_input, ensure_ascii=False
                            )
